				...
				const unsigned int digitOrder)
{
	...
	if (threadIdx.x < numBins)
	{
		int idxHist = threadIdx.x * gridDim.x + blockIdx.x;
		d_hist[idxHist] = s_hist[threadIdx.x];
				...
				const dim3 blockSizeScatter)
{
	...
	for (unsigned int i = 0; i <= maxBits; i += numBits)
	{
		...
		//histogram 
		timer.Start();
		unsigned int* d_hist = host_histogram(pInVals, numElems, numBins, mask, i, blockSizeHist);
		timer.Stop();
...
{ 
	...
	const dim3 blockSizeScatter(256); //smaller to better
